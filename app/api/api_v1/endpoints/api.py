...
<<<<<<< HEAD
=======
from app.schemas.ai import ExamRequest, ExamResponse
...
async def generate_exam(request: ExamRequest, db=Depends(get_database)):
    """Genera un examen completo con IA"""
    try:
        questions = await gemini_service.generate_mock_exam(
            area=request.area,
            topics=request.topics
        )
        return ExamResponse(success=True, questions=questions)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


# 1. Asegúrate de importar los nuevos esquemas arriba:

# 2. Agrega esta ruta nueva:
@router.post("/generate-exam", response_model=ExamResponse)
async def generate_exam(request: ExamRequest):
    """Genera examen de 100 preguntas"""
    try:
        questions = await gemini_service.generate_mock_exam(request.area, request.topics)
        return ExamResponse(success=True, questions=questions)
    except Exception as e:
>>>>>>> 9a223186a942271eb8696222b87d6c994a655e83
        raise HTTPException(status_code=500, detail=str(e))